...
export default function PressureDropReport(props){
    ...
    return (
        ...
        <div style={{backgroundColor:"white"}}>
         <h5 style={{color:'black'}}>{props.paragraph.replaceAll("_",".") + " Πτώση πίεσης"}</h5>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr"}}>
            {
                props.inspection.measurements[props.paragraph].values.map((branch,branchIndex)=>{
                    return (
                        <>

                            <div style={{padding:"1rem",border:"2px solid black",margin:"1rem"}}>

                                <p style={{color:'black'}}><b>{"Μέτρηση κλάδου " + (branchIndex+1).toString()+": "}</b>{branch}</p>
